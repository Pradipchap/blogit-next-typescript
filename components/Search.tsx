  ...
  return (
    <div className="px-2 py-4 flex flex-col gap-2 min-h-52 w-96 bg-white">
      {loading || searchString === "" ? (
        ...
        <p className="w-full text-center self-center">No results</p>
      ) : (
        data?.blogs.map(({ _id, title, image, date, description }) => {
          return (
            <Link
              href={`/blogs/${_id}`}
              key={_id}
              className="flex items-center w-full h-16 gap-2 py-2 hover:bg-slate-50 transition-all rounded-lg px-2 cursor-pointer"
            >
              <Image
                src={image}
                height={75}
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-xs w-full font-medium line-clamp-1 text-gray-400">
                  {description}
                </p>
              </div>
            </Link>
